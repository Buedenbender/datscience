...
[`table1`](https://cran.r-project.org/web/packages/table1/vignettes/table1-examples.html) (Benjamin Rich), 
and [`flextable`](https://ardata-fr.github.io/flextable-book/) (David Gohel and Colleagues).
Really all credit should go to teams maintainig these two packages.  
...
So what am I actually talking about, see for yourself below! 
...
We will primarly use the `datscience` package. As well as a bit of `tidyverse` and `labelled` to
...
First, we will changing the column Classification to a factor and assigning the labels obtained
from the UCI Repository. Additionally we will generate a new fictional variable called `random_Smoking` with 3 associated factor levels `c("Smoker","Non-Smoker","Occasionally")`.  
...
# For markdown we additionally need to fix the autofit property, this step is not
